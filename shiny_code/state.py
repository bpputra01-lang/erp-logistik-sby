...
# ==========================================================================
    ...
    def run_so_step1(self, f_scan, f_stock, sub_sel, bin_sys_sel):
        try:
            ...
            # Helper pembersih dasar teks string
            def clean_str(val):
            ...
            def get_base_numeric_key(s):
                ...
                if re.match(r'^\d+(\.\d+)?$', s):
                    if '.' in s:
                        ...
                        return f"{depan_clean if depan_clean != '' else '0'}.{belakang}"
                    else:
                        s_clean = s.lstrip('0')
                        return s_clean if s_clean != "" else "0"
                return s

            # 1. Bersihkan & Filter Stock System Terlebih Dahulu (Sebagai Master Acuan)
            dt = df_t_raw.copy()
            col_b, col_s, col_q_sys = dt.columns[1], dt.columns[2], dt.columns[9]
            ...
            dt[col_b] = dt[col_b].astype(str).str.strip().str.upper()
            dt[col_s] = dt[col_s].apply(clean_str)
            dt[col_q_sys] = pd.to_numeric(dt[col_q_sys], errors='coerce').fillna(0)
            ...
            for s_sys in system_skus_set:
                if s_sys:
                    k = get_base_numeric_key(s_sys)
                    base_to_system_sku[k] = s_sys

            # 2. Bersihkan Data Scan & SESUAIKAN DENGAN FORMAT STOCK SYSTEM
            ds = df_s_raw.iloc[:, [0, 1, 2]].copy()
            ds.columns = ['BIN', 'SKU', 'QTY_SCAN']
            ds['BIN'] = ds['BIN'].astype(str).str.strip().str.upper()
...
# ==========================================================================
    ...
    def process_balancing_stock(self, f_stock, f_sales, sub_filter=None):
        try:
            ...
            # Filter Sub Kategori (Kolom G / Indeks 6) pada Stock jika dipilih
            ...
            df_valid_stk = pd.DataFrame({
                ...
                'SUB_KATEGORI': col_sub_raw[mask_valid] if isinstance(col_sub_raw, pd.Series) else "-",
