...
# CSS Fix: Tulisan input HIJAU NEON & Background gelap (Anti-Silau)
st.markdown("""
    <style>
...
st.title("🚛 JEZ LOGISTICS & WAREHOUSE SBY")
st.markdown("---")

# --- 3. SECTION: INPUT DATA MASTER (TIM BARU) ---
st.header("👤 1. Input Database Tim")
...
# --- 4. SECTION: INPUT LIBUR & MONITORING ---
st.header("🚫 2. Plot Libur & Cek Status")
col_l1, col_l2 = st.columns([1, 2])

with col_l1:
    df_k = pd.read_sql_query("SELECT nama FROM karyawan", conn)
    with st.form("form_libur", clear_on_submit=True):
        target = st.selectbox("Pilih Nama Tim", df_k['nama']) if not df_k.empty else st.warning("Isi Database Tim Dulu!")
...
with col_l2:
    st.write("**Monitoring Libur Terbaru**")
...
# --- 5. SECTION: DAFTAR TIM AKTIF (BIAR BISA CEK TIPE & ROLE) ---
with st.expander("🔍 LIHAT DAFTAR TIM TERDAFTAR (NAMA, TIPE, ROLE)", expanded=True):
...
# --- 6. SECTION: GENERATOR JADWAL ---
st.header("🚀 3. Generate Jadwal Mingguan")
c_g1, c_g2 = st.columns([1, 4])

with c_g1:
    start_date = st.date_input("Senin Mulai", datetime.now())
    enable_so = st.checkbox("Aktifkan Shift 3 (Stock Opname)", value=False) # Fitur Shift 3 SO
    ...
    if st.button("RUN GENERATOR", use_container_width=True):
        ...
        # Susunan Role Sesuai Screenshot
        roles = [
        ...
        for i, d_str in enumerate(days):
            ...
            for shf_name, pos in roles:
                ...
                for k in kandidat:
                    ...
                    # Part-Full boleh 2 shift per hari (Anti-Double untuk tipe lain)
...
if 'res_df' in st.session_state:
    ...
    st.dataframe(st.session_state.res_df.style.applymap(style_final), use_container_width=True, height=700)
    
    csv = st.session_state.res_df.to_csv(index=False)
    st.download_button("📥 DOWNLOAD CSV", csv, f"Jadwal_{start_date}.csv", "text/csv")
