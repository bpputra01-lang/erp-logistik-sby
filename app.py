...
if menu == "📊 Dashboard Overview":
    st.markdown("""
        <style>
            /* 1. Paksa Container Streamlit mentok ke samping */
            ...
            /* 2. Container Dashboard dengan tinggi yang JAUH (Mengatasi Merge Cell) */
            .merge-cell-fix {
                width: 100%;
                height: 3500px; /* Kita tembak tinggi ekstrem biar data di bawah merge cell keluar */
                background: #0e1117;
            }
            iframe {
                width: 100%;
    ...
    # Gunakan mode 'rm=minimal' biar Google gak repot ngerender menu atas mereka
    url_final = f"{dash_links[pilih_dash]}&rm=minimal&chrome=false"

    st.markdown(f'<div class="merge-cell-fix"><iframe src="{url_final}"></iframe></div>', unsafe_allow_html=True)
