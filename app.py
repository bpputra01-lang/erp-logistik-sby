...
def execute_ultra_fast_logistics_matching(file_sby, file_smg, file_sales, file_toc):
    """
    Engine pemrosesan data logistik (Python Streamlit x Pandas).
    Sudah diselaraskan 100% presisi dengan logic Macro VBA Excel.
    """
    # 1. AMBIL DATA SEBAGAI STRING (dtype=str) - Anti-Crash Nuklir 'NaN'
    df_sby = pd.read_excel(file_sby, dtype=str)
    df_smg = pd.read_excel(file_smg, dtype=str)
    df_sales = pd.read_excel(file_sales, dtype=str)
    df_toc = pd.read_excel(file_toc, dtype=str)
    
    # Trim spasi di nama kolom biar ga ada mismatch
    for df in [df_sby, df_smg, df_sales, df_toc]:
        df.columns = df.columns.str.strip()

    # ==========================================
    # 2. PROSES SHEET STOCK SURABAYA (Master)
    # ==========================================
    sku_col_sby = df_sby.columns[2]   # Kolom C -> SKU
    name_col_sby = df_sby.columns[4]  # Kolom E -> Item Name
    var_col_sby = df_sby.columns[5]   # Kolom F -> Variant
    qty_col_sby = df_sby.columns[9]   # Kolom J -> Qty (Sesuai arrSby(i, 10) di VBA)
    
    # Bersihkan SKU & QTY (Setara CDbl + On Error Resume Next di VBA)
    df_sby[sku_col_sby] = df_sby[sku_col_sby].fillna('').astype(str).str.strip()
    df_sby[qty_col_sby] = pd.to_numeric(df_sby[qty_col_sby], errors='coerce').fillna(0)
    
    # Grouping (Sum Qty Akumulasi & Ambil First Item Name/Variant sesuai logic VBA)
    ...
    # Filter internal: hapus baris yang SKU-nya kosong ("")
    sby_grouped = sby_grouped[sby_grouped['SKU'] != '']

    # ==========================================
    # 3. PROSES SHEET STOCK SEMARANG 
    # ==========================================
    sku_col_smg = df_smg.columns[2]   # Kolom C -> SKU
    qty_col_smg = df_smg.columns[9]   # Kolom J -> Qty
    
    df_smg[sku_col_smg] = df_smg[sku_col_smg].fillna('').astype(str).str.strip()
    ...
    # ==========================================
    # 4. PROSES SHEET SALES 60d
    # ==========================================
    sku_col_sales = df_sales.columns[17] # Kolom R -> SKU
    qty_col_sales = df_sales.columns[25] # Kolom Z -> Sales Qty
    
    df_sales[sku_col_sales] = df_sales[sku_col_sales].fillna('').astype(str).str.strip()
    ...
    # ==========================================
    # 5. PROSES SHEET ToC (Lookup Master)
    # ==========================================
    art_col_toc = df_toc.columns[2] # Kolom C -> Article / Item Name
    toc_col_toc = df_toc.columns[7] # Kolom H -> ToC
    
    df_toc[art_col_toc] = df_toc[art_col_toc].fillna('').astype(str).str.strip()
    
    # SELARAS MACRO VBA: Menggunakan keep='last' karena dictionary VBA menimpa data lama (Last Row Win)
    toc_lookup = df_toc[[art_col_toc, toc_col_toc]].drop_duplicates(subset=[art_col_toc], keep='last')
    toc_lookup.columns = ['ITEM NAME', 'ToC']

    # ==========================================
    # 6. PENYUSUNAN HASIL COMPARE (LEFT JOIN)
    # ==========================================
    # Loop/Merge berdasarkan Unique SKU dari Surabaya (Sama seperti For Each key In dictSby.Keys)
    compiled = pd.merge(sby_grouped, smg_grouped, on='SKU', how='left')
    compiled = pd.merge(compiled, sales_grouped, on='SKU', how='left')
    
    # Isi data kosong hasil left join dengan 0 (Setara pengecekan .Exists di VBA)
    ...
    # FILTER LOGIC: Jika Surabaya DAN Semarang bernilai 0, SKIP (qtySby <> 0 Or qtySmg <> 0)
    compiled = compiled[(compiled['QTY SURABAYA'] != 0) | (compiled['QTY SEMARANG'] != 0)]

    # Lookup ke tabel ToC berdasarkan ITEM NAME
    final_df = pd.merge(compiled, toc_lookup, on='ITEM NAME', how='left')
    final_df['ToC'] = final_df['ToC'].fillna('-') # If Not dictToc.Exists Then "-"

    # Konversi hasil akhir kolom angka ke Integer Bulat murni
    final_df['QTY SURABAYA'] = final_df['QTY SURABAYA'].round().astype('int64')
    final_df['QTY SEMARANG'] = final_df['QTY SEMARANG'].round().astype('int64')
    final_df['SALES 60d'] = final_df['SALES 60d'].round().astype('int64')

    # Susun urutan 7 kolom sesuai format output VBA
...
def logic_setup_karantina_with_compare(df_master):
    def clean_val(x):
        if pd.isna(x): return ""
        s = str(x).strip().upper()
        if s.startswith("SPE"): 
    ...
    # Bersihkan nama kolom dari spasi dan paksa huruf besar
    ...
    for _, row in df_master.iterrows():
        try:
            # Ambil data menggunakan nama kolom langsung (Anti-geser / Anti-salah index)
            bin_raw = row.get('BIN', '')
            sku_raw = row.get('SKU', '')
            ...
            # Ambil QTY berdasarkan Nama Kolom di Excel kamu
            ...
            if diff != 0:
                ...
                # Gunakan abs(diff) agar nilai minus/plus tetap terhitung jumlah kuantitasnya di Karantina
   ...
   # --- INITIALIZE ALL SESSION STATES ---
    ...
    with col_f2:
        list_brand = [
            ...
            "LOTTO", "KRONIKEL", "ADIDAS", "VOOLA", "RECOIR", "MIZUNO", "UNKNOWN", "WARRIOR","AVO","KANKY"
        ]
    ...
    if df_recon_missing is not None and not df_recon_missing.empty:
        ...
        for _, row in df_recon_missing.iterrows():
            ...
            if s_recon in inbound_skus_set:
                ...
                single_list.append({'BIN': row[col_bin_miss], 'SKU': row[col_sku_miss], 'QTY ADJ': qty_calc})

    # D. PIVOT & MERGE KE MASTER INBOUND
...
def logic_setup_real_plus(df_stock_final, df_multiple_adj_plus, df_recon_missing=None):
    ...
    # 1. Cukup jadikan SET untuk mencatat SKU apa saja yang valid/diizinkan (Approved SKU)
    # Tidak perlu simpan BIN asalnya di sini karena rawan tertimpa kalau multi-bin
    allowed_skus = set()
    if not df_multiple_adj_plus.empty:
        # Cari kolom SKU di df_multiple_adj_plus (biasanya kolom ke-2 atau ke-3 tergantung struktur master)
        col_sku_multi = df_multiple_adj_plus.columns[2] if len(df_multiple_adj_plus.columns) > 2 else df_multiple_adj_plus.columns[0]
        allowed_skus = set(df_multiple_adj_plus[col_sku_multi].apply(clean_val).unique())
    ...
    for i in range(len(df_stock)):
        if qty_so.iloc[i] > qty_system.iloc[i]:
            ...
            # Ambil QTY Selisih murni dari baris stock ini (bukan dari total pivot)
            qty_mutasi = diff_val.iloc[i] 
            
            # Validasi: Apakah SKU ini ada di list inbound/multiple?
            if sku_key in allowed_skus:
                # Untuk "BIN AWAL", kita ambil default atau dinamis dari staging inbound 
                # karena di multiple_adj data aslinya sudah melebur jadi satu master inbound.
                bin_awal = "STAGING INBOUND" 
                
                setup_real_data.append({
                    "BIN AWAL": bin_awal,
                    "BIN TUJUAN": bin_tujuan,
                    "SKU": sku_key,
                    "QUANTITY": qty_mutasi, # Hasilnya akan presisi: Baris A1-1 dapet 10, Baris A1-2 dapet 5
                    "NOTES": "MISS LOCATION"
                })
                seen_entry.add(f"{sku_key}|{bin_tujuan}")
    ...
    if df_recon_missing is not None and not df_recon_missing.empty:
        for _, row_m in df_recon_missing.iterrows():
            bin_tujuan_m = row_m.iloc[0] 
            sku_raw_m = row_m.iloc[1]    
            sku_key_m = clean_val(sku_raw_m)
            qty_m = pd.to_numeric(row_m.iloc[6], errors='coerce') or 0

            if sku_key_m in allowed_skus and f"{sku_key_m}|{bin_tujuan_m}" not in seen_entry:
                setup_real_data.append({
...
def clean_and_process_sales(df_sales):
    ...
    # Konversi kolom S (QTY - Index 18) menjadi numerik
    ...
    # Kunci Logika Seleksi Keyword Kolom A
    store_upper = df[col_store].astype(str).str.upper()
    df["SALES_ONLINE"] = np.where(
        store_upper.str.contains("ONLINE", na=False), df[col_qty], 0
    )
    df["SALES_OFFLINE"] = np.where(
        store_upper.str.contains("JEZ", na=False), df[col_qty], 0
    )

    # Grouping per Unique SKU
    df_summary = (
        df.groupby(col_sku)
        .agg({"SALES_ONLINE": "sum", "SALES_OFFLINE": "sum"})
        .reset_index()
    )

    df_summary["TOTAL_SALES"] = (
        df_summary["SALES_ONLINE"] + df_summary["SALES_OFFLINE"]
    )
    return df_summary
...
def generate_stock_allocation(df_stock, df_sales_summary):
    ...
    # Grouping Stock per SKU
    df_stock_unique = (
        df_stk.groupby(col_sku).agg({col_qty: "sum"}).reset_index()
    )

    # Satukan data stock unique dengan summary sales
    sales_sku_col = df_sales_summary.columns[0]
    df_merged = pd.merge(
        df_stock_unique,
        df_sales_summary,
        left_on=col_sku,
        right_on=sales_sku_col,
        how="left",
    ).fillna(0)
    ...
    df_merged["QTY_ONLINE"] = np.ceil(
        df_merged[col_qty] * df_merged["PCT_ONLINE"]
    ).astype(int)
    df_merged["QTY_OFFLINE"] = np.ceil(
        df_merged[col_qty] * df_merged["PCT_OFFLINE"]
    ).astype(int)
    df_merged["QTY_LOGISTIK"] = (
        df_merged[col_qty] - df_merged["QTY_ONLINE"] - df_merged["QTY_OFFLINE"]
    )

    # Proteksi Penyesuaian Stok
    ...
    df_merged[["QTY_ONLINE", "QTY_OFFLINE", "QTY_LOGISTIK"]] = df_merged.apply(
        adjust_ceil_overflow, axis=1
    )
    ...
    # FIX: Baris return harus menjadi baris TERAKHIR di fungsi ini!
    return df_merged[output_cols]


# ==============================================================================
# UI STREAMLIT MAIN FUNCTION
# ==============================================================================


def menu_allocation_stock():
    """Interface Khusus untuk Menu Alokasi Stok Gudang (Online, Offline,
    Logistik)
    """
...
""", unsafe_allow_html=True)
    ...
    with st.expander("💡Logic Thinking"):
        st.info(
            ...
            " persentase alokasi tiap SKU berdasarkan performa rasio penjualan"
            " 90 hari terakhir."
        )
    ...
    with col1:
        file_stock = st.file_uploader(
            "Pilih file Excel Stock (.xlsx)", type=["xlsx"], key="stock"
        )
    with col2:
        file_sales = st.file_uploader(
            "Pilih file Excel Sales (.xlsx)", type=["xlsx"], key="sales"
        )
        ...
        try:
            ...
            if st.button("▶️ RUN PROCESS", use_container_width=False):
                with st.spinner(
                    ...
                    df_final_allocation = generate_stock_allocation(
                        df_stock_raw, df_sales_summary
                    )

                    st.session_state["df_final_allocation"] = (
                        df_final_allocation
                    )
            ...
            if st.session_state.get("processed", False):
                ...
                col_stock_qty = df_final_allocation.columns[1]
                total_stock = df_final_allocation[col_stock_qty].sum()

                with m1:
                    st.markdown(
                        "<div class='metric-card'><div"
                        " class='metric-title'>Total Stock Master</div><div"
                        f" class='metric-value'>{total_stock:,.0f} Pcs</div></div>",
                        unsafe_allow_html=True,
                    )
                with m2:
                    st.markdown(
                        "<div class='metric-card'><div"
                        " class='metric-title'>Alokasi Area Online</div><div"
                        " class='metric-value'>"
                        f"{df_final_allocation['QTY_ONLINE'].sum():,.0f} Pcs</div></div>",
                        unsafe_allow_html=True,
                    )
                with m3:
                    st.markdown(
                        "<div class='metric-card'><div"
                        " class='metric-title'>Alokasi Area Offline</div><div"
                        " class='metric-value'>"
                        f"{df_final_allocation['QTY_OFFLINE'].sum():,.0f} Pcs</div></div>",
                        unsafe_allow_html=True,
                    )
                with m4:
                    st.markdown(
                        "<div class='metric-card'><div"
                        " class='metric-title'>Alokasi Area"
                        " Logistik</div><div class='metric-value'>"
                        f"{df_final_allocation['QTY_LOGISTIK'].sum():,.0f} Pcs</div></div>",
                        unsafe_allow_html=True,
                    )
                ...
                def convert_df_to_excel(df):
                    ...
                    with pd.ExcelWriter(
                        ...
                        df.to_excel(
                            writer,
                            index=False,
                            sheet_name="Stock_Allocation_Result",
                        )
            ...
            st.error(
                f"Terjadi kesalahan pembacaan kolom struktur file: {str(e)}"
            )
            st.info(
                "💡 Pastikan urutan kolom di Excel Anda sudah sesuai: Stock"
                " (B, C, J) & Sales (A, AA, S)"
            )
    else:
        st.session_state["processed"] = False
        st.info(
            "💡 Silakan upload kedua file Excel di atas terlebih dahulu untuk"
            " memulai perhitungan alokasi."
        )
            ...
            with st.spinner("⚡ Mengompilasi data transaksi & analisis kalkulasi untuk SEMUA SKU... Harap tunggu..."):
                ...
                for sku in list_sku:
                    ...
                    if df_temp is not None and not df_temp.empty:
                       ...
                       # --- 2. Logic Fallback Baru (ANTI ZONK + AUTO SUM MULTIPLE ROW) ---
                        try:
                            # Bersihin target SKU dari dropdown (bikin uppercase + string)
                            ...
                            # Filter semua baris yang beneran cocok sama SKU target
                            matched_rows = df_all_stock[sku_column_cleaned == sku_target]
                            
                            if not matched_rows.empty and df_all_stock is not None:
                                # Ambil semua data di Kolom J (Index 9) untuk SKU tersebut
                                qty_series = matched_rows.iloc[:, 9]
                                
                                # Bersihin data kolom J, ubah ke numeric (kalau ada blank/NaN dipaksa jadi 0)
                                qty_numeric = pd.to_numeric(qty_series.astype(str).str.strip(), errors='coerce').fillna(0)
                                
                                # 🔥 SIKAT TOTAL NYA DI-SUM DISINI COK!
                                stock_system_calc = int(qty_numeric.sum())
                            else:
                                stock_system_calc = 0
                                
                        except Exception as e:
                            # Biar ketahuan di terminal kalau lo salah format data
                            print(f"❌ DEBUG ERROR SUM STOCK UTK SKU {sku_clean}: {e}")
                            stock_system_calc = 0
...
def process_koli_consolidation(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for target in target_bins:
        ...
        # CASE 1: Pasangkan dengan 1 BIN AMBIL
            ...
            if b1['QTY'] == needed_qty:
                ...
                if len(combined_skus) <= 2:
                    consolidation_results.append({
                        ...
                        'QTY TUJUAN': target['QTY'],
                        'SKU AMBIL': ", ".join(b1['SKUS']),
                        'BIN AMBIL': b1['BIN'],
                        'QTY AMBIL': b1['QTY'],
                        'SKU AMBIL 2': '-',
                        'BIN AMBIL 2': '-',
        ...
        # CASE 2: Pasangkan dengan Kombinasi 2 BIN AMBIL
...
# === TAMBAHKAN DI SINI (DI BAWAH METRIC BOX, DI ATAS COLUMNS) ===
    ...
    if not df_filtered_count.empty:
        ...
        total_not_genap = len(bg[bg.apply(
...
with st.sidebar:
    ...
    if is_dc:
        m3_list = ["Stock Opname","Match Real & System","Compare System","Cycle Count","Putaway & Picking Audit List", "List Bin Cycle Count", "Stock Tracking Timeline", "Justification SO", "Stock Minus", "List Retur Out", "Pengajuan Mutasi Karantina", "Refill Koli to Koli/Refill", "Stock Allocation"]
    else:
