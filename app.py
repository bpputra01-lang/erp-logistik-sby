...
if up_r4 and up_s4:
    if st.button("▶️ JALANKAN LOOKUP & DIFF", use_container_width=True):
        try:
            # Memperbaiki ValueError dengan smart read
            df_r4 = read_uploaded_file(up_r4)
            df_s4 = read_uploaded_file(up_s4)
...
if st.session_state.get('step4_done'):
    ...
    # PERBAIKAN DOWNLOAD KECIL (Point 2)
    # Kita tambahkan download button manual karena tombol bawaan sering tidak muncul
    with t_f: 
        df_res = st.session_state.df_res_lookup
        st.dataframe(df_res, use_container_width=True, hide_index=True)
        st.download_button("📥 Download Final Adjustment", df_res.to_csv(index=False).encode('utf-8'), "final_adj.csv", "text/csv")
        
    with t_m: 
        df_miss = st.session_state.df_missing_lookup
        st.dataframe(df_miss, use_container_width=True, hide_index=True)
        st.download_button("📥 Download Missing Items", df_miss.to_csv(index=False).encode('utf-8'), "missing_items.csv", "text/csv")
    ...
    if up_m5:
        if st.button("▶️ GENERATE ADJ +", use_container_width=True):
            try:
                df_m5 = read_uploaded_file(up_m5)
                
                # PERBAIKAN LOGIKA (Point 3)
                # Pastikan data yang masuk ke logic_pivot hanya yang valid (bukan missing)
                # Kita kirimkan df_res_lookup yang sudah dipastikan isinya sinkron
                ...
                st.error(f"Error pada Step 5: {e}")
    ...
    if st.session_state.get('step5_done'):
        ...
        with t_mult: 
            ...
            st.download_button("📥 Download Multiple Adj", st.session_state.df_mult_5.to_csv(index=False).encode('utf-8'), "multiple_adj.csv", "text/csv")
            
        with t_sing: 
            st.dataframe(st.session_state.df_sing_5, use_container_width=True)
            st.download_button("📥 Download Single Adj", st.session_state.df_sing_5.to_csv(index=False).encode('utf-8'), "single_adj.csv", "text/csv")
