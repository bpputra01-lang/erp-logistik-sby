...
with st.expander("🔍 Staff Database", expanded=True):
    # Asumsi data lu dari SQL
    df_cek = pd.read_sql_query("SELECT nama, posisi, tipe FROM karyawan", conn)
    
    if not df_cek.empty:
        for i, row in df_cek.iterrows():
            # Rasio [10, 1] biar area tombol makin sempit, otomatis dia mengecil
            c_card, c_del = st.columns([10, 1])
            
            with c_card:
                st.markdown(f"""
                    <div class="staff-card-dark">
                        <div class="card-title">{row['nama']}</div>
                        <div class="card-subtitle">{row['posisi']} • {row['tipe']}</div>
                    </div>
                """, unsafe_allow_html=True)
            
            with c_del:
                # Masukkan ke container 'small-del-container' buat nembak CSS-nya
