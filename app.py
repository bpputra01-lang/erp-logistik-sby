...
def menu_Stock_Opname():
    ...
    # 2. SEKSI FILTER
    with st.container():
        st.markdown('<p style="font-weight: bold; color: #1d3567;">🎯 FILTER DATA (HAPUS DATA TIDAK TERPILIH)</p>', unsafe_allow_html=True)
        ...
        with col_f1:
            ...
            # Jika tidak dipilih, berarti SEMUA. Jika dipilih, hanya yang dipilih.
            selected_sub = st.multiselect("🗂️ Sub Kategori (Stock System):", list_sub_kat)
        ...
        with col_f2:
            ...
            selected_bin_sys = st.multiselect("🏭 BIN System (Stock System):", list_bin_stock)
        ...
        with col_f3:
            ...
            selected_bin_cov = st.multiselect("📡 BIN Coverage (Data Scan):", list_bin_cov)
        ...
        if st.button("▶️ RUN COMPARE", use_container_width=True):
            try:
                ...
                with st.spinner("Memproses filter dan menghitung ulang..."):
                    ...
                    # --- FILTER 1 & 2: PADA STOCK SYSTEM (df_t_raw) ---
                    
                    # Filter Sub Kategori (Kolom G / Index 6)
                    # Jika selected_sub kosong, JANGAN HAPUS APAPUN (pertahankan semua)
                    if selected_sub:
                    ...
                    if selected_bin_sys:
                        # Membuat pola regex dari daftar yang dipilih
                        # Kita cek apakah Kolom B zaw Contiene salah satu nilai yang dipilih
                    ...
                    # --- FILTER 3: PADA DATA SCAN (df_s_raw) ---
                    
                    # Filter BIN Coverage (Jika dipilih)
                    # Hanya baris di Data Scan yang BIN-nya ada di daftar Coverage yang dipilih
                    if selected_bin_cov:
                    ...
                    # ======================================================
                    if df_t_raw.empty:
                        st.error("❌ Data System kosong setelah difilter! Tidak ada data yang cocok.")
                    elif df_s_raw.empty:
                        st.warning("⚠️ Data Scan kosong setelah difilter! Pastikan pilihan Coverage benar.")
                    else:
                        # --- JALANKAN LOGIKA COMPARE ---
                        
                        # Compare 1: Scan vs System (Cari REAL +)
                        res_scan = logic_compare_scan_to_stock(df_s_raw, df_t_raw)
                        
                        # Compare 2: System vs Scan (Cari SYSTEM +)
                        ...
                        if res_scan is not None and res_stock is not None:
                            ...
                            st.success(f"✅ Compare Berhasil! System difilter & Scan difilter Coverage.")
                        else:
                            st.error("❌ Gagal mendapatkan hasil compare.")

            except Exception as e:
                st.error(f"❌ Error Detail: {e}")

    # ======================================================
    # 5. TAMPILKAN HASIL (JIKA SUDAH ADA DI SESSION STATE)
    # ======================================================
