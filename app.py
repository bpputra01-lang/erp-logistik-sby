...
# --- 1. SQLITE LOGIC (Hanya Tambahan Tanpa Ubah UI) ---
...
def sync_daily_reset():
    ...
    if not res or res[0] != today:
        # RESET HARIAN: Status balik Belum, Todo hapus semua
        ...
        if not res:
            ...
            conn.execute('UPDATE reset_tracker SET last_date = ?', (today,))
        conn.commit()
    conn.close()

# Jalankan init & reset tracker
init_db()
sync_daily_reset()
