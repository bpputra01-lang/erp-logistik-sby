...
def menu_retur_out_system():
    ...
    st.markdown("""
        ...
        .metric-card { background-color: #1E1E2E; padding: 18px; border-radius: 12px; color: white; margin-bottom: 15px; border-left: 5px solid #007BFF; }
    ...
    if uploaded_file:
        try:
            ...
            if all(k in df_up.columns for k in mapping.keys()):
                ...
                st.error("Kolom file gak sesuai!")
        except Exception as e:
    ...
    try:
        ...
        if not df_db.empty:
            ...
            # Tampilkan Tabel
            event = st.dataframe(
                df_display.drop(columns=['rowid', 'id'], errors='ignore'),
            ...
            if event.selection.rows:
                ...
                if st.button(f"🗑️ HAPUS SKU {df_display.iloc[idx]['sku']}", type="primary"):
                    conn.execute("DELETE FROM retur_out WHERE rowid = ?", (int(t_id),))
                    conn.commit()
                    st.rerun()
        else:
            st.info("💡 Database Kosong. Silakan upload file dulu.")
